...
StatusType world_cup_t::unite_teams(int teamId1, int teamId2, int newTeamId)
{
    ...
    try {
        ...
        Tree<PlayerRank> merged_ranks = Tree<PlayerRank>(
                sortedArray2Tree(merged_rank_array, 0, players1 + players2 - 1));
        merged_players.inOrder(updatePlayerIdPointers, 0);
        ...
        if (new_team->isComplete()) {
            ...
            if (prevComInList != nullptr && completeTeamList.getSize() > 0) {
                completeTeamList.insertAfter(prevComInList->data.getCompleteNode(),
                                             completeTeams.find(*new_complete_team)->data);
            } else {
